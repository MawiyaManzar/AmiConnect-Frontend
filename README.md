...
- Dependencies: `flask`, `psycopg2-binary`, `python-dotenv`, `bcrypt`, `pyjwt`

### Installation
1. **Clone the Repository:**
   ```bash
   git clone https://github.com/your-repo/amiconnect.git
   cd amiconnect
   ```
...
2. **Set Up Environment:**
   - Create a `.env` file:
     ```
     DATABASE_URL=postgresql://postgres:<your_password>@db.pklkbcfxfzropaoelvrl.supabase.co:5432/postgres?sslmode=require
...
4. **Apply Database Schema:**
   - Use Supabase's SQL Editor or `psql` to run `schema.sql`:
     ```bash
...
1. **Signup:**
   ...
   curl -X POST http://localhost:5000/signup -H "Content-Type: application/json" -d '{
     "name": "John Doe",
     "email": "john@s.amity.edu",
     "password": "securepassword",
     "gender": "Male",
     "department": "BTech",
     "year": 2,
     "connection_type": "Study Partner",
     "interests": ["Web Development", "Data Science", "Cybersecurity & Ethical Hacking"],
...
3. **Verify Database:**
   - Use Supabase's Table Editor or `psql`:
     ```sql
     SELECT * FROM users;
     ```
